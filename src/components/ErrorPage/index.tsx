...
const ErrorPage: FC<ErrorPageProps> = ({restricted=false}) => {
  ...
  return <div className={styles['container']}>
    ...
    <div className={restricted?styles['flexRowContainer']:styles['flexColContainer']}>
      <button onClick={()=>router.back()} className={styles['btnSecondary']}>Go Back</button>
      <button onClick={()=>router.replace(user?.isAuthenticated?"/community":"/")} className={styles['btnPrimary']}>Home Page</button>
    </div>
  </div>
}
