...
const Places: React.FC<Props> = ({ data: initialData }) => {
  ...
  const fetchMoreData = useCallback(async () => {
    ...
    if (page_type && page_type !== 'All') {
      ...
      queryString =
        `type=2&page_type=${encodeURIComponent(category.toString())}&` +
        queryString
    } else {
      queryString = `type=2&` + queryString
    }
  ...
  return (
    <>
      <Head>
        ...
        />
        <meta property="og:image" content="/place_page.png" />
        <meta property="og:image:secure_url" content="/place_page.png" />
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  if (query['page-type'] && query['page-type'] !== 'All') {
    ...
    queryString =
      `type=2&page_type=${encodeURIComponent(query.category.toString())}&` +
      queryString
  } else {
    queryString = `type=2&` + queryString
  }
