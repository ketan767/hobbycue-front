...
const CommunityLinks: React.FC<Props> = ({}) => {
  ...
  const { allPosts,allPostsWithLink, loading, filters } = useSelector(
  ...
  //     `page=1&limit=500&has_link=true&populate=_author,_genre,_hobby,_allHobbies._hobby1,_allHobbies._hobby2,_allHobbies._hobby3,_allHobbies._genre1,_allHobbies._genre2,_allHobbies._genre3&sort=-up_votes.count`,
  //   )
  //   if (filters.hobby !== '') {
    ...
    <>
      <CommunityPageLayout activeTab="links">
        <section className={styles['pages-container']}>
          ...
          ) : allPostsWithLink?.length > 0 ? (
            allPostsWithLink.map((post: any) => {
              return (
                <PostCard
                  key={post._id}
                  postData={post}
