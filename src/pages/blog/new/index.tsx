...
import BlogComments from '../[url]/Comments'
import {
  ...
  return (
    <>
      <Head>
        ...
        <title>{`${data?.blog_url?.title || 'New Blog'} | HobbyCue`}</title>
      </Head>
        ...
        <div className={styles['blog-header']}>
          ...
          {isEditing ? (
            <textarea
              ...
              ref={taglineRef}
              onKeyDown={(e) => e.key === 'Enter' && taglineRef.current?.blur()}
              rows={1}
              onInput={function (e) {
                const target = e.target as HTMLTextAreaElement
                target.style.height = 'auto'
            ...
            data?.blog_url?.tagline && (
              <h1 className={styles['blog-desc']}>{data?.blog_url?.tagline}</h1>
            )
          )}
              ...
              <div className={styles['author-details']}>
                ...
                <div className={styles['date-and-hobbies']}>
                  <p>
                    ...
                    !isNaN(new Date(data?.blog_url?.publish_date).getTime())
                      ? dateFormatwithYear.format(
                          new Date(data?.blog_url?.publish_date),
                        )
                      : data?.blog_url?.createdAt &&
                        !isNaN(new Date(data?.blog_url?.createdAt).getTime())
                      ? dateFormatwithYear.format(
            ...
            {/* <div className={styles.profileAndComment}>
              <div className={styles['profile-wrapper']}>
                <div className={`${styles['header-user']}`}>
                  {user?.profile_image ? (
                    <Link
                      href={`/profile/${user?.profile_url}/blogs`}
                  ...
                  <div className={styles['title']}>
                    ...
                    <p>
                      <span>
                        {!isNaN(
                          new Date(data?.blog_url?.publish_date).getTime(),
                        )
                          ? dateFormatwithYear?.format(
                              new Date(data?.blog_url?.createdAt),
                            )
